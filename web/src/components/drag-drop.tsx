...
export const DragDrop = ({ selectedFile, setSelectedFile }: DragDropProps) => {
  ...
  return (
    <div {...getRootProps()} className="w-4/5 max-w-xl rounded-lg border-black border py-24 flex justify-center bg-slate-200">
      {isDragActive ? <p className="text-slate-600">Drop here...</p> : renderDropedFileName()}
    </div>
  )
}
